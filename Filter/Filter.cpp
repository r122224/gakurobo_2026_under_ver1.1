...
double Filter::stepFilter(double input){

     D = sqrt(pow(k2,2) - 4 * k1 * k2);
     a1 = (0.5 * k2) + (0.5 * D);
     a2 = (0.5 * k2) - (0.5 * D);
     Gs = ((k1 * k2) / D )* ((1 / (input + a1)) - (1 / (input + a2)));
    double Ys = (1 / input) - (((k1 * k2)/(a1 * D)) * (1 / (input + a1))) + (((k1 * k2)/(a2 * D)) * (1 / (input + a2)));


     vref = int_time - ((k1 * k2 * exp(-a1 * int_time))/(a1 * D)) + ((k1 * k2 * exp(-a2 * int_time))/(a2 * D));
    //double Output = 1;
